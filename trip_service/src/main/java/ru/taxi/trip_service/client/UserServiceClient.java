...
public class UserServiceClient {
    ...
    public void updateDriverStatus(String driverId, DriverStatus newStatus) {
        Map<String, DriverStatus> body = Map.of("status", newStatus);
        restTemplate.patchForObject(
                userServiceUrl + "/api/drivers/{id}/status",
                body, Void.class, driverId);
    }

    public List<PassengerDto> getAllPassengers() {
        log.info("Отправка запроса к сервису: {}/api/passengers", userServiceUrl);
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                log.error("Ошибка: статус {}", response.getStatusCode());
                return Collections.emptyList();
            }
        } catch (RestClientException e) {
            log.error("Не удалось получить пассажиров: {}", e.getMessage(), e);
            return Collections.emptyList();
        }
    }
    ...
    public Optional<PassengerDto> getPassenger(String passengerId) {
        try {
            PassengerDto passenger = restTemplate.getForObject(
                    userServiceUrl + "/api/passengers/{id}", PassengerDto.class, passengerId);
